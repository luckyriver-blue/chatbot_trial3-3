...
#Firestoreのデータへのアクセス
ref = db.collection("users3_3").document(st.session_state["user_id"]).collection("conversation").order_by("timestamp")
    ...
    for i, message in enumerate(st.session_state["messages"]):
        if message["role"] == "human":
            ...
            if i == len(st.session_state["messages"]) - 1 and st.session_state["interval"] != None:
                ...
                remaining = 4 - elapsed.total_seconds()
                if remaining > 0:
...
add_ref = db.collection("users3_3").document(st.session_state["user_id"]).collection("conversation")
#送信ボタンが押されたとき
def send_message():
